...
#_(fiber.impl/excise conn [17592186733493 17592186733477 17592186054375 17592186054375] )

(defn batch-tx
  [file-in  conn & {:keys [line-num] :or {line-num 10000}}]
  (with-open [reader (io/reader  file-in)]
    (let [total-lines (fiber.impl/count-lines file-in)
          tx-count (atom 0)
          raw (line-seq reader)
...
#_(fiber.impl/count-lines "/opt/data/stage1/nut-data.edn")
#_(fiber.impl/count-total (db-now) :usda.nutrdata/id)
...
(comment
  ...
  (fiber.impl/q-attrs (db-now))

  (fiber.impl/q-idents (db-now))
  ...
  (fiber.impl/count-total (db-now) :usda.nutr/desc )
  (fiber.impl/get-paginted-entity
   {:db (db-now)
    :attribute :usda.nutr/desc
    :limit 10
    :offset 0})
  
  (fiber.impl/count-total (db-now) :usda.item/id)
  (fiber.impl/get-paginted-entity
   {:db (db-now)
